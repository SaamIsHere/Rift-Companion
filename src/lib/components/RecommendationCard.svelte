...
>
  ...
  <div class="hidden sm:flex flex-1 items-center gap-1.5 min-w-0 px-2 overflow-hidden">
    <!-- Good Matchup: only when enemies in draft actually match -->
    {#if rec.good_matchups && rec.good_matchups.length > 0}
      <div
        class="inline-flex items-center gap-1.5 rounded-lg border border-emerald-500/35 bg-emerald-950/40 px-2 py-0.5 text-xs shadow-sm min-w-0 max-w-full shrink overflow-hidden"
      ...
      >
        ...
        <span class="truncate min-w-0 text-emerald-200/90 font-medium">
    ...
    {#if rec.bad_matchups && rec.bad_matchups.length > 0}
      <div
        class="inline-flex items-center gap-1.5 rounded-lg border border-rose-500/35 bg-rose-950/40 px-2 py-0.5 text-xs shadow-sm min-w-0 max-w-full shrink overflow-hidden"
      ...
      >
        ...
        <span class="truncate min-w-0 text-rose-200/90 font-medium">
    ...
    {#if rec.synergies && rec.synergies.length > 0}
      <div
        class="inline-flex items-center gap-1.5 rounded-lg border border-purple-500/35 bg-purple-950/40 px-2 py-0.5 text-xs shadow-sm min-w-0 max-w-full shrink overflow-hidden"
      ...
      >
        ...
        <span class="truncate min-w-0 text-purple-200/90 font-medium">
          {rec.synergies.join(", ")}
        </span>
      </div>
    {/if}
  ...
  <div class="shrink-0 pl-1.5 z-10">
    <button
      type="button"
      on:click={handleMoreInfo}
      class="group/btn inline-flex items-center gap-1 rounded-lg border border-purple-500/30 bg-purple-600/20 px-2.5 py-1.5 text-xs font-semibold text-purple-200 transition-all hover:border-purple-400 hover:bg-purple-600/40 hover:text-white active:scale-95 whitespace-nowrap"
